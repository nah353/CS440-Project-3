/**
 * Gemini Service - Business Logic Layer
 * Handles AI-powered media analysis
 */
...
export const GeminiService = {
  async analyzeMedia(fileBuffer, mimeType) {
    try {
      // Double-check API key
      if (!process.env.GEMINI_API_KEY) {
        throw new Error(
          "GEMINI_API_KEY is not set. Please configure it in server/.env"
        );
      }
      ...
      const prompt = `You are a recipe expert. Identify the food in this ${
      ...
      if (!jsonMatch) {
        console.error(
          "Could not extract JSON from response. Full response:",
          text
        );
        throw new Error(
          "API response did not contain valid JSON. Response: " +
      ...
      try {
        ...
        throw new Error(
          "Could not parse API response as JSON: " + parseError.message
        );
      }
      ...
      if (!recipeData.title || !recipeData.instructions) {
        throw new Error(
          "Recipe missing required fields (title or instructions)"
        );
      }
